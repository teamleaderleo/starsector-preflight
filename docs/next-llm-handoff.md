...
Next engineering work is to review the retained nonfatal GraphicsLib/ShaderLib log diagnostics and decide what controlled baseline, if any, is needed before default enablement or timing work.
...
Review the captured diagnostics and determine whether to:

1. classify them as known profile-level noise using existing evidence;
2. add a narrowly scoped compatibility-mode log-baseline runner; or
