...
DIAGNOSTIC_PROPERTY="-Dpreflight.preparedPixels.coherentOriginalConvert=true"
...
if [[ ! -f preflight-agent/src/main/java/dev/starsector/preflight/agent/TexturePreparedPixelCarrierSurface.java ]]; then
    echo "This checkout does not contain the coherent-carrier diagnostic." >&2
...
CONTRACT_REPORT="$REPORT_DIR/prepared-pixel-coherent-converter-contract-$STAMP.json"
PREP_REPORT="$REPORT_DIR/prepared-pixel-coherent-converter-preparation-$STAMP.json"
RUN_DIR="$HOME/.starsector-preflight/runs/prepared-pixel-coherent-converter-probe-$STAMP"
mkdir -p "$REPORT_DIR"
...
jq -e '
    .successful == true
    and .readiness.preparedPixelsNextOperatorAction == "launcher-only-coherent-original-converter-probe"
...
echo
echo "== Exact launcher-only diagnostic plan =="
JAVA_TOOL_OPTIONS="$DIAGNOSTIC_JAVA_TOOL_OPTIONS" \
    java -jar "$JAR" "${RUN_ARGS[@]}" --dry-run

echo
echo "This run bypasses ImageIO for NPOT cache hits but deliberately keeps"
echo "Starsector's original pixel converter, buffer, side effects, and cleanup."
echo
echo "When the launcher appears:"
echo "  1. Confirm whether the background, buttons, borders, toggles, and text look normal."
echo "  2. Take a screenshot."
echo "  3. DO NOT click Play or start Starsector."
echo "  4. Close the launcher with its X button."
echo
read -r -p "Press Enter to start the single coherent-converter probe, or Ctrl-C to stop: " _
...
echo
echo "== Starting launcher-only coherent-converter probe =="
set +e
JAVA_TOOL_OPTIONS="$DIAGNOSTIC_JAVA_TOOL_OPTIONS" \
...
echo
echo "== Coherent-converter telemetry =="
jq '.textureCompatibility.preparedPixels' "$RUN_DIR/adapter.json"

echo
echo "== Original converter observations =="
...
jq -e '
    ...
    and .textureCompatibility.preparedPixels.coherentOriginalConvertEnabled == true
    and .textureCompatibility.preparedPixels.coherentCarriers > 0
    ...
    and all(
        .textureCompatibility.preparedPixels.originalLayoutObservations[];
        .coherentOriginalConvert == true
        and .carrierRasterWidth == .sourceWidth
        and .carrierRasterHeight == .sourceHeight
        and .carrierSampleModelWidth == .sourceWidth
        and .carrierSampleModelHeight == .sourceHeight
    )
...
echo
echo "Automated lifecycle, coherent-carrier, original-converter, observation, and cleanup checks passed."
echo "The screenshot decides which hypothesis survives."
echo
echo "Upload both:"
echo "  $ARCHIVE_OUT"
