...
CONTRACT_REPORT="$REPORT_DIR/prepared-pixel-coherent-direct-contract-$STAMP.json"
PREP_REPORT="$REPORT_DIR/prepared-pixel-coherent-direct-preparation-$STAMP.json"
RUN_DIR="$HOME/.starsector-preflight/runs/prepared-pixel-coherent-direct-probe-$STAMP"
mkdir -p "$REPORT_DIR"
...
jq -e '
    .successful == true
    and .readiness.preparedPixelsNextOperatorAction == "launcher-only-coherent-direct-probe"
...
echo
echo "This run uses the coherent source-sized cached image together with"
echo "the direct cached upload buffer and cached derived colors for NPOT hits."
echo "It does not execute Starsector's original image decoder or pixel converter"
echo "for admitted prepared-cache hits."
echo
...
echo
read -r -p "Press Enter to start the single coherent-direct probe, or Ctrl-C to stop: " _
...
echo
echo "== Starting launcher-only coherent-direct probe =="
set +e
JAVA_TOOL_OPTIONS="$DIAGNOSTIC_JAVA_TOOL_OPTIONS" \
...
echo
echo "Automated lifecycle, coherent-direct, and cleanup checks passed."
